...
import { User, Mail, Shield, MapPin, Save, Grid, Clock, ChevronRight, LogOut, Package } from "lucide-react";
        ...
        const fetchProfile = async () => {
            try {
                ...
                setProfileData({
                    name: data.name,
                    email: data.email,
                    address: data.address || { street: "", city: "", zip: "", country: "", phone: "" }
                });
            } catch (error) {
                console.error(error);
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-20 px-4 relative font-sans text-slate-900">
            ...
            <div className="max-w-6xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl shadow-slate-200/50 border border-white/60 text-center relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <div className="w-32 h-32 mx-auto bg-slate-900 rounded-full p-1 shadow-2xl shadow-slate-900/30 mb-6">
                                <div className="w-full h-full bg-slate-800 rounded-full flex items-center justify-center text-5xl font-black text-slate-200">
                            ...
                            <div className="space-y-3">
                                <button className="w-full flex items-center gap-3 px-5 py-4 bg-slate-900 text-white rounded-xl font-bold text-sm shadow-lg shadow-slate-900/20 transition-transform hover:scale-[1.02]">
                                    <User className="w-5 h-5" /> My Profile
                                </button>
                                <button onClick={() => navigate("/my-orders")} className="w-full flex items-center gap-3 px-5 py-4 bg-white hover:bg-slate-50 text-slate-600 border border-slate-100 rounded-xl font-bold text-sm transition-all hover:shadow-md">
                                    <Package className="w-5 h-5" /> My Orders
                                    <ChevronRight className="w-4 h-4 ml-auto opacity-50" />
                                </button>
                                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-5 py-4 bg-red-50 hover:bg-red-100 text-red-600 border border-red-100 rounded-xl font-bold text-sm transition-colors">
                ...
                >
                    {/* Intro */}
                    <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-xl shadow-slate-200/40 border border-white/60 flex flex-col md:flex-row justify-between md:items-center gap-4">
                        <div>
                    ...
                    <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            disabled
                                            className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3.5 pl-10 text-sm font-bold text-slate-500 cursor-not-allowed"
                                        />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2 space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-700 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all shadow-sm"
                                    />
                                </div>
                                <div className="space-y-2">
                        ...
                        <div className="md:col-span-2 flex justify-end pt-4">
                            <button
                                type="submit"
                                disabled={saving}
                                className="px-10 py-4 bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-2xl font-bold text-lg hover:shadow-xl hover:shadow-slate-900/20 hover:-translate-y-1 transition-all flex items-center gap-3 disabled:opacity-70 disabled:hover:translate-y-0"
                            >
                                {saving ? (
                                    <>
